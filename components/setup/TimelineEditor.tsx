    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center mt-4 border-t border-gray-200 pt-4">
                <h3 className="text-lg font-semibold text-gray-700">{listTitle}</h3>
                <div className="flex bg-gray-100 rounded-lg p-1 text-xs">
                    <button
                        type="button"
                        onClick={() => setViewMode('visual')}
                        className={`px-3 py-1 rounded-lg transition-all ${viewMode === 'visual' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'}`}
                    >
                        Timeline
                    </button>
                    <button
                        ...
                        className={`px-3 py-1 rounded-lg transition-all ${viewMode === 'list' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'}`}
                    >
                        List
                    </button>
                </div>
            ...
            {viewMode === 'visual' && entries.length > 0 ? (
                    ...
                    <div className="space-y-8">
                        {sortedEntries.map((entry, index) => (
                            <div key={entry.id} className={`relative flex items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'} flex-row`}>
                                {/* Node on the line */}
                                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-4 h-4 bg-white border-2 border-indigo-400 rounded-full z-10 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
                                ...
                                <div className={`w-full md:w-1/2 pl-10 md:pl-0 ${index % 2 === 0 ? 'md:pr-8 text-left' : 'md:pl-8 text-left'}`}>
                                    <div className="bg-gray-50/40 border border-gray-300/50 backdrop-blur-sm p-4 rounded-lg hover:border-indigo-500/50 transition-all group relative">
                                        <input
                                            type="text"
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-transparent font-bold text-gray-900 placeholder-slate-500 focus:outline-none text-sm mb-1"
                                        />
                                        <textarea
                                            ...
                                            rows={2}
                                            className="w-full bg-transparent text-xs text-gray-600 placeholder-slate-600 focus:outline-none resize-none"
                                        />
                                        <button
                                            type="button"
                ...
                <div className="space-y-3">
                    {entries?.map((entry, index) => (
                        <div key={entry.id} className="p-3 bg-gray-100/50 rounded-lg space-y-2 relative border-l-4 border-indigo-500">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="w-1/3 bg-slate-600 text-xs font-mono text-indigo-200 placeholder-slate-400 rounded-lg p-2 border border-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                                />
                                <input
                                    type="text"
                                    placeholder={t('common.name')}
                                    ...
                                    className="w-2/3 bg-slate-600 font-bold text-slate-100 placeholder-slate-400 rounded-lg p-2 border border-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                                />
                            </div>
                            <textarea
                                placeholder={t('common.description')}
                                ...
                                rows={2}
                                className="w-full bg-slate-600 text-gray-900 placeholder-slate-400 rounded-lg p-2 border border-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                            />
                            <button type="button" onClick={() => onRemove(index)} className="absolute top-2 right-2 p-1.5 text-gray-600 hover:text-rose-400"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                    ))}
                </div>
            )}
