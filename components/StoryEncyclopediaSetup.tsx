    ...
    return (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl max-w-md w-full p-6 space-y-6">
                 ...
                 {isError ? (
                     <div className="bg-red-900/30 text-red-200 p-4 rounded-md border border-red-800 text-sm">
                         {error}
                     </div>
                 ) : (
                    <div className="space-y-3">
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.basic ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : <SpinnerIcon className="w-5 h-5 text-indigo-400" />}
                            <span className={steps.basic ? "text-emerald-400" : "font-medium"}>{t('setup.autoBuild.stepBasic')}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.core ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : (steps.basic ? <SpinnerIcon className="w-5 h-5 text-indigo-400" /> : <div className="w-5 h-5 rounded-full border-2 border-slate-600" />)}
                            <span className={steps.core ? "text-emerald-400" : (steps.basic ? "font-medium" : "text-slate-500")}>{t('setup.autoBuild.stepCore')}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.world ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : (steps.core ? <SpinnerIcon className="w-5 h-5 text-indigo-400" /> : <div className="w-5 h-5 rounded-full border-2 border-slate-600" />)}
                            <span className={steps.world ? "text-emerald-400" : (steps.core ? "font-medium" : "text-slate-500")}>{t('setup.autoBuild.stepWorld')}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.relations ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : (steps.world ? <SpinnerIcon className="w-5 h-5 text-indigo-400" /> : <div className="w-5 h-5 rounded-full border-2 border-slate-600" />)}
                            <span className={steps.relations ? "text-emerald-400" : (steps.world ? "font-medium" : "text-slate-500")}>{t('setup.autoBuild.stepRelations')}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.arc ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : (steps.relations ? <SpinnerIcon className="w-5 h-5 text-indigo-400" /> : <div className="w-5 h-5 rounded-full border-2 border-slate-600" />)}
                            <span className={steps.arc ? "text-emerald-400" : (steps.relations ? "font-medium" : "text-slate-500")}>{t('setup.autoBuild.stepArc')}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-300">
                            {steps.tone ? <CheckIcon className="w-5 h-5 text-emerald-400" /> : (steps.arc ? <SpinnerIcon className="w-5 h-5 text-indigo-400" /> : <div className="w-5 h-5 rounded-full border-2 border-slate-600" />)}
                            <span className={steps.tone ? "text-emerald-400" : (steps.arc ? "font-medium" : "text-slate-500")}>{t('setup.autoBuild.stepTone')}</span>
                        </div>
                    </div>
                 )}
  ...
  const handleAutoBuild = async () => {
      ...
      try {
          ...
          if (showWorldBuilding || showMagicSystem) {
             ...
             if (showMagicSystem) {
                 const msRes = await generateStoryEncyclopediaSection(apiKey, 'magicSystem', currentData, contentLanguage);
          ...
          if (currentData.characters.length >= 2) {
             const relRes = await generateStoryEncyclopediaSection(apiKey, 'relationships', currentData, contentLanguage);
             currentData = { ...currentData, relationships: relRes.relationships || [] };
             setFormData(currentData);
          }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'format') {
        ...
        if (selectedFormat && (!formData.totalChapters || !formData.wordsPerChapter)) {
             setFormData(prev => ({
                 ...prev,
                 format: value as 'novel' | 'webnovel',
                 totalChapters: prev.totalChapters || selectedFormat.defaultChapters,
                 wordsPerChapter: prev.wordsPerChapter || selectedFormat.defaultWords
             }));
             return;
        }
    }
    ...
    <div className="w-full p-4">
      ...
      <div className="max-w-4xl mx-auto py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-b border-slate-700 flex space-x-1 sticky top-[72px] bg-slate-900 z-10 py-1 -mx-2 px-2 overflow-x-auto whitespace-nowrap scrollbar-hide w-full flex-nowrap">
              {TABS.map(tab => ( 
                  <button 
                    ...
                    className={`flex-shrink-0 px-4 py-2 text-sm font-medium rounded-t-md transition-colors ${ activeTab === tab.id ? 'bg-slate-800 border-slate-700 border-b-0 border-l border-r border-t text-indigo-400' : 'bg-transparent text-slate-400 hover:bg-slate-800/60' }`}
                  > 
                    {t(tab.label)} 
                  </button>
              ))}
            ...
            <div className="pt-4">
            {activeTab === 'basic' && <FormSection 
                title={t('setup.tabs.basic')} 
                onGenerate={() => handleGenerate('basic')} 
                generateDisabled={!isBasicInfoReady && !isEditing} 
                isGenerating={generatingSection === 'basic'}
                actions={
                    !isEditing && (
                        <button 
                            type="button" 
                            ...
                            className={`flex items-center gap-2 px-3 py-1.5 text-sm font-bold text-white rounded-md transition-all ${
                                isBasicInfoReady 
                            ...
                            title={isBasicInfoReady ? t('setup.autoBuild.buttonDesc') : "Please enter an idea and select a genre first."}
                        >
            ...
            {activeTab === 'world' && (
                <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
                    ...
                    {/* Sub-Tabs - FIXED LAYOUT FOR MOBILE */}
                    <div className="flex space-x-2 bg-slate-900/50 p-1 rounded-md mb-4 overflow-x-auto whitespace-nowrap no-scrollbar">
                        <button type="button" onClick={() => setActiveWorldTab('geo')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex-shrink-0 ${activeWorldTab === 'geo' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>{t('world.subTabs.geo')}</button>
                        <button type="button" onClick={() => setActiveWorldTab('nature')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex-shrink-0 ${activeWorldTab === 'nature' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>{t('world.subTabs.nature')}</button>
                        <button type="button" onClick={() => setActiveWorldTab('power')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex-shrink-0 ${activeWorldTab === 'power' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>{t('world.subTabs.power')}</button>
                        <button type="button" onClick={() => setActiveWorldTab('history')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex-shrink-0 ${activeWorldTab === 'history' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>{t('world.subTabs.history')}</button>
                    </div>
                    ...
                    <div key={index} className="p-4 bg-slate-700/50 rounded-md relative space-y-4 border border-slate-600/50"> 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 bg-slate-800/50 p-3 rounded-md">
                            <div>
                                ...
                                <input type="number" value={act.startChapter || ''} onChange={(e) => handleArcChange(index, 'startChapter', e.target.value)} placeholder="#" className="w-full bg-slate-700 text-slate-200 text-sm rounded-md p-1.5 border border-slate-600" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">{t('setup.arc.end')}</label>
                                <input type="number" value={act.endChapter || ''} onChange={(e) => handleArcChange(index, 'endChapter', e.target.value)} placeholder="#" className="w-full bg-slate-700 text-slate-200 text-sm rounded-md p-1.5 border border-slate-600" />
                            </div>
